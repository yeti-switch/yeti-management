...
cat >>Dockerfile <<EOF
RUN apt-get install --yes --no-install-recommends --force-yes pkg-config cmake pkg-kde-tools libprotobuf-dev protobuf-compiler libconfuse-dev libnanomsg-dev libsctp-dev libevent-dev
